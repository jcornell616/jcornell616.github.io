---
layout: post
title: "DSP Engineering Roadmap"
author: "Jackson Cornell"
categories: journal
tags: [dsp, resources, books, websites, blogs, videos]
image: /roadmap/roadmap.png
---
...
1. [What is DSP?](#intro)
2. [Web Resources](#resources)
3. [DSP Implementation](#implementation)
4. [Preliminaries](#preliminaries)
	1. [Complex Numbers](#complex-nums)
	2. [Calculus](#calculus)
	3. [Linear Algebra](#linear-algebra)
	4. [Signals & Systems](#signals-systems)
	5. [Digital Electronics](#digital-e)
	6. [Statistics & Probability](#stats-prob)
5. [Core Topics](#core)
	1. [DSP](#dsp)
	2. [Stochastic Processes](#stochastic-proc)
	3. [Image Processing](#image-proc)
	4. [Communications](#communications)
	5. [Machine Learning](#machine-learning)
6. [Related Topics](#related)
	1. [Control Theory](#controls)
	2. [RF Engineering](#rf-engineering)
	3. [Embedded Systems](#embedded-systems)
6. [Advanced Topics](#advanced)
	1. [Detection & Estimation Theory](#detection-estimation)
	2. [Multirate Signal Processing](#multirate)
	3. [Information Theory](#information-theory)
	3. [Convex Optimization](#convex-optimization)
	4. [Adaptive Filtering](#adaptive-filteirng)
	5. [Kalman Filtering](#kalman-filtering)
	6. [Spectrum Sensing](#spectrum-sensing)
	7. [Error Correcting Codes](#ecc)
	8. [Multidimensional Signal Processing](#multidimensional-proc)
	9. [VLSI for DSP](#vlsi-dsp)
	10. [Sparse Signals](#sparse-signals)
	11. [Graph Signals](#graph-signals)
	12. [RADAR](#radar)
	13. [SONAR](#sonar)
	14. [Audio Processing](#audio-proc)
	15. [Computer Vision](#computer-vision)
	16. [Biomedical Applications](#biomed-apps)
	17. [Compression](#compression)
	18. [Speech Processing](#speech-proc)
...
In more general terms, the **Euclidean norm** on $\mathbb{C}^N$, otherwise know as the $L^2$ norm, is given as $\vert\vert \mathbf{x} \vert\vert_2 = (\sum_{n=0}^{N-1} \vert x_n \vert^2)^{1/2}$. For norms not induced by an inner product, there is also the **Manhatten norm** on $\mathbb{C}^N$, otherwise know as the $L^1$ norm, given as $\vert\vert \mathbf{x} \vert\vert_1 = \sum_{n=0}^{N-1} \vert x_n \vert$. The $L^{\infty}$ norm defined on $\mathbb{C}^N$ is defined as $\vert\vert \mathbf{x} \vert\vert_{\infty} = \text{max} (\vert x_n \vert)$, or the element of largest absalute value. These norms are further generalized to the notion of the $L^{p}$ norm given as $\vert\vert \mathbf{x} \vert\vert_p = (\sum_{n=0}^{N-1} \vert x_n \vert^p)^{1/p}$ where $p$ is a real-valued scalar greater than or equal to 1. If the $L^{p}$ norm is not given (such as $\vert\vert \cdot \vert\vert$), it can be assumed that it is refering to the $L^{2}$ norm. [[2]](#ref2)
...
*Figure 4: Relationship between different function spaces [[2]](#ref1)*
...
| $\mathcal{l}^{\infty}(\mathbb{Z})$ |  Set of infinite sequences $(x_1, x_2, ...)$ of real or complex values such that $ \text{max}_i( \vert x_i \vert) < \infty$ |
