...
In more general terms, the **Euclidean norm** on $\mathbb{C}^N$, otherwise know as the $L^2$ norm, is given as $\vert\vert \mathbf{x} \vert\vert_2 = (\sum_{n=0}^{N-1} \vert x_n \vert^2)^{1/2}$. For norms not induced by an inner product, there is also the **Manhatten norm** on $\mathbb{C}^N$, otherwise know as the $L_1$ norm, given as $\vert\vert \mathbf{x} \vert\vert_1 = \sum_{n=0}^{N-1} \vert x_n \vert$. The $L_{\infty}$ norm defined on $\mathbb{C}^N$ is defined as $\vert\vert \mathbf{x} \vert\vert_{\infty} = \text{max}_{n=0, 1, ..., N-1} \vert x_n \vert$, or the element of largest absalute value. These norms are further generalized to the notion of the $L_{p}$ norm given as $\vert\vert \mathbf{x} \vert\vert_p = (\sum_{n=0}^{N-1} \vert x_n \vert^p)^{1/p}$ where $p$ is a real-valued scalar greater than or equal to 1. If the $L_{p}$ norm is not given (such as $\vert\vert \cdot \vert\vert$), it can be assumed that it is refering to the $L_{2}$ norm. [[2]](#ref2)
